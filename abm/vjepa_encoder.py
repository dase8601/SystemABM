...
Note: DINOv3 (6x more training data) is gated on HuggingFace and requires
manual approval. DINOv2 is freely available and already produces discriminative
features (cos_sim(black,white) ≈ 0.62 vs V-JEPA's collapsed 0.997).
...
class VJEPAEncoder:
    ...
    # ImageNet normalization (same as V-JEPA)
    ...
    def __init__(self, device: str = "cuda", img_size: int = 224):
        ...
        with torch.no_grad():
            ...
            if bw_sim > 0.90:
                print("  WARNING: features not discriminative!")
            else:
